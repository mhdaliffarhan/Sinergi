<template>
  <div>
    <div v-if="!item.dokumenTerkait" class="flex items-center justify-between p-3 rounded-lg bg-gray-50 dark:bg-gray-800/60 border-l-4 border-yellow-400">
      ...
      <div class="flex items-center gap-2">
        ...
        <button @click="$emit('unggah', item.id)" class="px-3 py-1 text-xs font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700">
          Unggah
        </button>
      </div>
    </div>
    ...
    <div 
      v-else
      class="flex items-center justify-between p-3 rounded-lg border-l-4 transition-all duration-300"
